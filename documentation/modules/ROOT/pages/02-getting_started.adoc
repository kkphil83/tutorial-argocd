...
OpenShift 콘솔의 개발자 화면에서, *Topology* 메뉴로 들어가서 방금 동기화한 애플리케이션의 웹 주소로 들어갈 수 있습니다. (각자 계정의 argocd-* 프로젝트를 선택하세요!)
...
`Application` 매니페스트를 수정하여 구성 드리프트를 자동으로 고치도록 구성할 수 있습니다. 가장 아래쪽의 `selfHeal` 기능을 true로 변경하는 것입니다.
