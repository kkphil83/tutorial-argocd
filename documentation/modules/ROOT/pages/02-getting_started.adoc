...
[NOTE]
====
환경 설명: retail-%userid% 프로젝트에 Argo CD 서버가 설치되어 있습니다. 이를 톻애 argocd-%userid% 프로젝트에 샘플 애플리케이션을 배포할 것입니다.
====
...
Argo CD 웹 UI를 살펴보면 Argo가 응용 프로그램을 "Out of Sync"으로 감지한 것을 볼 수 있습니다.

image::out-of-sync.png[Out of Sync]

Argo CD 웹 userid를 통해 앱을 동기화하는 방법은 다음과 같습니다

* 먼저 화면 상단의 `SYNC` 버튼 클릭
* 그리고 `SYNCHRONIZE` 버튼 클릭

또는 wsl 환경에서 oc login 이후 다음 명령을 실행할 수도 있습니다.
...
동기화 프로세스가 완료되면 Argo CD UI에서 애플리케이션이 동기화되었다고 표시됩니다.

image::fullysynced.png[Fully Synced]

애플리케이션이 실행 중인 탭에서 페이지를 다시 로드합니다. 원래대로 파란색 거품으로 돌아왔을 것입니다.

image::bgd.png[BDG App]

`Application` 매니페스트를 설정하여 드리프트를 자동으로 수정하도록 Argo CD를 설정할 수 있습니다. 가장 아래쪽의 `selfHeal` 기능을 *true*로 변경하는 것입니다.
