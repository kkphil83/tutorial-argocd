...
[NOTE]
====
Red Hat은 이러한 확장 기능을 설치하는 데 필요한 기능을 지원하지만, Red Hat은 현재는 특정한 타사의 확장 기능을 지원하지 않습니다. 확장 기능을 사용해도 OpenShift GitOps에 대한 지원을 받으실 수 있지만, 타사 확장 기능 자체는 지원하지 않습니다.
====
...
이전 과정에서는 롤아웃의 기본 개념을 실습하기 위해 `Rollout` 이 `ReplicaSet` 을 제어하였습니다. *Rollout Extension* 을 실습하기 위해 이번에는 'Deployment' 를 배포하고, 함께 포함된 'Rollout' 이 해당 'Deployment' 를 참조하여 워크로드의 배포 전략을 제어하는 `Application` 을 만듭니다.
...
애플리케이션의 업데이트에 대해 `Rollout Extension` 은 어떻게 보여주는지 확인하겠습니다.
...
Rollout Extension은 웹 UI에서 롤아웃을 프로모션(승격)시키거나 중단(롤백)시키는 기능을 제공합니다.
