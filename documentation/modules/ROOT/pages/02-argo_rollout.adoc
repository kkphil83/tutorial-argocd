...
그런 다음 롤아웃은 트래픽의 20%를 애플리케이션의 신규 버전인 카나리아 버전으로 라우팅하고, 수동 프로모션을 기다린 다음, 전체 트래픽을 새 애플리케이션 버전으로 라우팅할 때까지 여러 번의 자동 프로모션을 수행합니다.
...
<5> Argo Rollouts 컨트롤러에 45초 동안 해당 가중치를 유지하라고 지정합니다. 기간 값은 초( s), 분( m) 또는 시간( h)으로 설정할 수 있습니다. 예를 들어, 1시간 *1h* 동안 지정할 수 있습니다. 값을 지정하지 않으면 기간 값은 기본적으로 초 단위로 설정됩니다.
<6> 생성할 Pod를 지정합니다.
...
우리가 배포한 샘플 애플리케이션은 Argo Rollouts위 이해를 돕기 위한 웹 화면을 제공합니다.

OpenShift 콘솔의 운영자 메뉴에서 Networking → Routes 를 클릭하고 `rollout` Route의 웹 주소를 클릭합니다.

image::rollout-6.png[rollout]

애플리케이션의 컨테이너 이미지 태그인 `blue` 색상으로, 사용자 요청이 들어오는 모습을 표시해주는 웹 서비스 화면이 보입니다.
...
이미지 태그를 수정하면 롤아웃 업데이트가 진행되며, 이전 단계에서 실행해두었던 롤아웃의 진행상황을 확인하는 터미널에서 새로운 revision이 생성되었습니다.
...
우리가 생성했던 롤아웃의 전략은 카나리아 버전으로 초기에 20%의 트래픽을 보내면서 프로모션(승격) 요청이 올 때까지 롤아웃이 무기한 멈추는 것이었습니다.
...
롤아웃이 일시 중지 상태이므로 운영자는 이제 롤아웃을 수동으로 승격시켜 다음 단계로 진행해야 합니다.
...
기존에 실행했던 롤아웃 상태를 확인했던 터미널을 확인해보면, 카나리아 버전에서 트래픽 가중치가 40%로 증가합니다. 지정된 시간이 지나면서 트래픽 가중치가 점차 이동합니다.
...
다음 명령을 수행하여 컨테이너 이미지 태그를 수정하여 롤아웃 업데이트를 진행시킵니다.
