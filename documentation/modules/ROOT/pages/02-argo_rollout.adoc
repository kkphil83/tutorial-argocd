...
`Rollout` 메뉴에서 Current namespace only (현재 네임스페이스만) 를 선택하고 `Create Rollout` 또는 `Argo Rollout 만들기` 버튼을 누릅니다.

image::rollout-2.png[rollout]

`YAML보기`를 선택하여 Yaml 편집기에 아래의 내용을 붙여 넣고 하단의 `Create` (만들기) 버튼을 눌러서 롤아웃 개체를 생성합니다.
...
기존에 실행했던 롤아웃 상태를 확인하는 *터미널1*을 확인해보면, 카나리아 버전에서 트래픽 가중치가 40%로 증가합니다. 지정된 시간이 지나면서 트래픽 가중치가 점차 이동합니다.
